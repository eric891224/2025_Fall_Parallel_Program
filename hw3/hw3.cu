...
#pragma unroll
  for (int i = 0; i < C.md_iter; ++i) {
    ...
    if (r > C.bailout) break;  // if escaped
  }
  ...
  for (int i = 0; i < C.ray_step; ++i) {
    len = map(ro + rd * t, trap,
              ID);  // get minimum distance from current ray position to the
                    // object's surface
    if (glm::abs(len) < C.eps || t > C.far_plane) break;
...
int main(int argc, char** argv) {
  ...
  image = new unsigned char*[height];  // not used in device code
...
//                 vec3 cf = glm::normalize(ta - ro);  // forward vector
//                 vec3 cs =
//                     glm::normalize(glm::cross(cf, vec3(0., 1., 0.)));  //
//                     right (side) vector
//                 vec3 cu = glm::normalize(glm::cross(cs, cf));          // up
//                 vector vec3 rd = glm::normalize(uv.x * cs + uv.y * cu + FOV *
//                 cf);  // ray direction
//                 //---
...
//                 //---lighting
//                 vec3 col(0.);                          // color
//                 vec3 sd = glm::normalize(camera_pos);  // sun direction
//                 (directional light) vec3 sc = vec3(1., .9, .717);          //
//                 light color
//                 //---
...
//                     vec3 nr = calcNor(pos);              // get surface
//                     normal vec3 hal = glm::normalize(sd - rd);  //
//                     blinn-phong lighting model (vector
//                     // h)
//                     // for more info:
//                     //
//                     https://en.wikipedia.org/wiki/Blinn%E2%80%93Phong_shading_model
...
//                     shadow double dif = glm::clamp(glm::dot(sd, nr), 0., 1.)
//                     * sdw;   // diffuse double spe =
//                     glm::pow(glm::clamp(glm::dot(nr, hal), 0., 1.), gloss) *
//                         dif;  // self shadow

//                     vec3 lin(0.);
//                     lin += ambc * (.05 + .95 * amb);  // ambient color *
//                     ambient lin += sc * dif * 0.8;            // diffuse *
//                     light color * light intensity col *= lin;

//                     col = glm::pow(col, vec3(.7, .9, 1.));  // fake SSS
//                     (subsurface scattering) col += spe * 0.8; // specular
//                 }
//                 //---
...
//         // print progress
//         // printf("rendering...%5.2lf%%\r", current_pixel / total_pixel *
//         100.); current_pixel++;
//     }
// }